...
create table cidades (id bigint  auto_increment,  nome varchar (255) ) engine=InnoDB default charset = utf8;
                       
create table enderecos ( longadouro varchar (10), numero varchar (10),   bairro varchar (100),      cep varchar (20))engine=InnoDB default charset = utf8;
...
alter table contatos add constraint pessoa_id_fk foreign key(pessoa_id) references pessoas(id) on delete cascade on update action;


alter table endercos modify longradouro varchar (10) not null;
...
alter table enderecos add constraint pessoas_id_fk foreign key(pessoa_id) references pessoas(id) on delete cascade on update action;
alter table enderecos add constraint cidade_id_fk foreign key(cidade_id) references cidades(id) on delete cascade on update action;


alter table  estado modify sigla varchar (2) not null;
alter table estado modify nome varchar (100) not null;

alter table cidades modify nome varchar (255) not null;
alter table cidades add column estado_sigla varchar(2);
alter table cidades add constraint estado_sigla_fk foreign key(estado_sigla) references estado(sigla) on delete cascade on update action;
...
alter table lancamentos add constraint pessoas_id_fk foreign key (pessoa) references pessoas (id) on delete cascade on update actio;
alter table lancamentos add constraint categorias_id_fk foreign (categoria) references categorias (id) onde delete cascade on update action;
